...
module.exports = {
    command: ['menu'],
    description: 'show bot menu list',
    owner: false,
    group: false,
    private: false,
    limit: false,
    execute: async (m, { ednut, isOwner, command, isCmd, reply, addCountCmd, getPosiCmdUser, randomemoji, isCreator, example, quoted, reaction, text, fetchJson, args, botNumber, pushname, isGroup, isPrivate, pickRandom, prefix, ftroli, fkontak, limitUser, reply4, isPremium }) => {
let value = isPremium ? "✓" : "𐄂";
let timestampe = speed()
let latensi = speed() - timestampe
let a = global.db.users[m.sender].limit
let cash = global.db.users[m.sender].money
let me = m.sender
let { getDevice } = require('@whiskeysockets/baileys')
  if (m.isGroup) {
...
┃${global.simbol} ${global.prefix}tictactoe 
┃${global.simbol} ${global.prefix}delttt
┗━━━━━━━━━━━━━━━━
